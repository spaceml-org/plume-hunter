...
def main(options):
    ...
    train_scene_ids = np.load(
        os.path.join(ROOT_DIR, "scripts", "training", "scene_ids_train.npy")
    ).tolist()
    val_scene_ids = np.load(
        os.path.join(ROOT_DIR, "scripts", "training", "scene_ids_val.npy")
    ).tolist()
